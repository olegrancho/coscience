...
**Pending values render dimmed and italic** (`opacity: 0.45`) until the save
succeeds, so "adjusted but not yet written" is visible rather than assumed. The
dim clears when the server's own value comes back.
...
it visible without an extra click.
